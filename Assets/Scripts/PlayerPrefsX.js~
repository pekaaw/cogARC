...
enum ArrayTypeTwo {Float, Int32, Bool, String, Vector2, Vector3, Quaternion, Color}
...
static function SetBoolArray (key : String, boolArray : boolean[]) : boolean {
	...
	bytes[0] = System.Convert.ToByte (ArrayTypeTwo.Bool);	// Identifier
...
static function SetStringArray (key : String, stringArray : String[]) : boolean {
	...
	bytes[0] = System.Convert.ToByte (ArrayTypeTwo.String);	// Identifier
	Initialize();
...
static function GetStringArray (key : String) : String[] {
	if (PlayerPrefs.HasKey(key)) {
		...
		if (bytes[0] != ArrayTypeTwo.String) {
...
static function SetIntArray (key : String, intArray : int[]) : boolean {
	return SetValue (key, intArray, ArrayTypeTwo.Int32, 1, ConvertFromInt);
}
 
static function SetFloatArray (key : String, floatArray : float[]) : boolean {
	return SetValue (key, floatArray, ArrayTypeTwo.Float, 1, ConvertFromFloat);
}
 
static function SetVector2Array (key : String, vector2Array : Vector2[]) : boolean {
	return SetValue (key, vector2Array, ArrayTypeTwo.Vector2, 2, ConvertFromVector2);
}
 
static function SetVector3Array (key : String, vector3Array : Vector3[]) : boolean {
	return SetValue (key, vector3Array, ArrayTypeTwo.Vector3, 3, ConvertFromVector3);
}
 
static function SetQuaternionArray (key : String, quaternionArray : Quaternion[]) : boolean {
	return SetValue (key, quaternionArray, ArrayTypeTwo.Quaternion, 4, ConvertFromQuaternion);
}
 
static function SetColorArray (key : String, colorArray : Color[]) : boolean {
	return SetValue (key, colorArray, ArrayTypeTwo.Color, 4, ConvertFromColor);
}
 
private static function SetValue (key : String, array : IList, ArrayTypeTwo : ArrayTypeTwo, vectorNumber : int, convert : function(IList, byte[], int)) : boolean {
	if (array.Count == 0) {
		Debug.LogError ("The " + ArrayTypeTwo.ToString() + " array cannot have 0 entries when setting " + key);
		return false;
	}
	var bytes = new byte[(4*array.Count)*vectorNumber + 1];
	bytes[0] = System.Convert.ToByte (ArrayTypeTwo);	// Identifier
	Initialize();
...
static function GetIntArray (key : String) : int[] {
	...
	GetValue (key, intList, ArrayTypeTwo.Int32, 1, ConvertToInt);
...
static function GetFloatArray (key : String) : float[] {
	...
	GetValue (key, floatList, ArrayTypeTwo.Float, 1, ConvertToFloat);
...
static function GetVector2Array (key : String) : Vector2[] {
	...
	GetValue (key, vector2List, ArrayTypeTwo.Vector2, 2, ConvertToVector2);
...
static function GetVector3Array (key : String) : Vector3[] {
	...
	GetValue (key, vector3List, ArrayTypeTwo.Vector3, 3, ConvertToVector3);
...
static function GetQuaternionArray (key : String) : Quaternion[] {
	...
	GetValue (key, quaternionList, ArrayTypeTwo.Quaternion, 4, ConvertToQuaternion);
...
static function GetColorArray (key : String) : Color[] {
	...
	GetValue (key, colorList, ArrayTypeTwo.Color, 4, ConvertToColor);
...
private static function GetValue (key : String, list : IList, ArrayTypeTwo : ArrayTypeTwo, vectorNumber : int, convert : function(IList, byte[])) {
	if (PlayerPrefs.HasKey(key)) {
		var bytes = System.Convert.FromBase64String (PlayerPrefs.GetString(key));
		...
		if (bytes[0] != ArrayTypeTwo) {
			Debug.LogError (key + " is not a " + ArrayTypeTwo.ToString() + " array");
			return;
		}
		Initialize();
...
static function ShowArrayTypeTwo (key : String) {
	var bytes = System.Convert.FromBase64String (PlayerPrefs.GetString(key));
	if (bytes.Length > 0) {
		var ArrayTypeTwo : ArrayTypeTwo = bytes[0];
		Debug.Log (key + " is a " + ArrayTypeTwo.ToString() + " array");
	}
}
